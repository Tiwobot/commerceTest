      ...
      <div className="relative w-full h-screen overflow-y-auto overflow-x-hidden flex items-center justify-center">
        ...
        <div ref={contentRef} className="relative z-10 max-w-2xl mx-auto pt-2 pb-52 px-4 transition-transform duration-300 will-change-transform">
          ...
          <form className="flex flex-col gap-4 max-w-lg mx-auto" onSubmit={handleSubmit}>
            <input type="text" placeholder={t('contact.name')} value={name} onChange={e => setName(e.target.value)} required className="rounded px-4 py-2 bg-white/10 text-white placeholder:text-white/60 border border-white/30 focus:outline-none focus:ring-2 focus:ring-white/40 transition" />
            <input type="email" placeholder={t('contact.email')} value={email} onChange={e => setEmail(e.target.value)} required className="rounded px-4 py-2 bg-white/10 text-white placeholder:text-white/60 border border-white/30 focus:outline-none focus:ring-2 focus:ring-white/40 transition" />
            <textarea placeholder={t('contact.message')} rows={5} value={message} onChange={e => setMessage(e.target.value)} required className="rounded px-4 py-2 bg-white/10 text-white placeholder:text-white/60 border border-white/30 focus:outline-none focus:ring-2 focus:ring-white/40 transition" />
